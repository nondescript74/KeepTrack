...
func isGoalMet(goal: CommonGoal, previous: Int) -> Bool {
    ...
    let goalCantBeMet: Bool = !previousGoalDates.isEmpty && previousIntake < previousGoalDates.count
    logger.info( "goal cant be met: \(goalCantBeMet)")
    if goalCantBeMet { return false }
    
    let isGoalMet_no_goals: Bool = goalDates.isEmpty
    logger.info( "no goals: \(isGoalMet_no_goals)")
    
    let isGoalMet_no_more_goals: Bool = remainingGoalDatesFirst == nil || remainingGoalDates.isEmpty ? true : false
    ...
    let isGoalMet_with_previous_goals_equal_intake_plus_one: Bool = !previousGoalDates.isEmpty && previousGoalDates.count == previousIntake + 1
    logger.info( "previous goals equal intake plus one: \(isGoalMet_with_previous_goals_equal_intake_plus_one)")
    
    let isGoalMet_with_previous_intake_and_previous_goals: Bool = !previousGoalDates.isEmpty && previousIntake + 1 > previousGoalDates.count
    logger.info( "with previous intake and goals: \(isGoalMet_with_previous_intake_and_previous_goals)")
    
    if remainingGoalDatesFirst != nil {
        ...
        myResult = isGoalMet_no_goals || isGoalMet_no_more_goals || isGoalMet_with_previous_intake || isGoalMet_with_previous_goals_equal_intake_plus_one || isGoalMet_with_previous_intake_and_previous_goals || isGoalMet_with_first_remaining_using_hour || isGoalMet_with_first_remaining_using_hour_n_minute
