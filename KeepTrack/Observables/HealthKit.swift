...
//            let format = NSLocalizedString("UNDETERMINED_NUMBER_OF_TYPES", comment: "")
            ...
            healthStore.getRequestStatusForAuthorization(toShare: [HKQuantityType.quantityType(forIdentifier: .dietaryWater)!], read: [HKQuantityType.quantityType(forIdentifier: .dietaryWater)!]) { (authorizationRequestStatus, error) in
                ...
                if let error = error {
                    ...
                    case .shouldRequest:
                        ...
                        status = self.requestHealthDataType()
                        self.logger.info("\(status)")
                    case .unknown:
    ...
    func requestHealthDataType() -> String {
        let dataTypesToRequest: Set<HKSampleType> = [
            HKQuantityType.quantityType(forIdentifier: .dietaryWater)!]
        var status: String = ""
        
        healthStore.requestAuthorization(toShare: dataTypesToRequest, read: dataTypesToRequest) { (success, error) in
            if let error = error {
                    status = "HealthKit Authorization Error: \(error.localizedDescription)"
            } else {
                if success {
                    status = "HealthKit authorization successful."
                } else {
                    status = "HealthKit authorization failed."
                }
            }
        }
        return status
    ...
    func addWaterSample(quantity: HKQuantity) async {
        let request = requestHealthDataType()
        if request == "HealthKit authorization successful." {
    ...
    func requestWaterSamples(from startDate: Date, to endDate: Date) async -> [HKQuantitySample] {
        let request = requestHealthDataType()
        if request != "HealthKit authorization successful." {
            self.logger.info("\(request)")
            return []
